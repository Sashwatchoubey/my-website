      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-4xl my-8">
        ...
        <div className="p-6 overflow-y-auto max-h-[75vh]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
            ...
            {/* Working Days / Roster */}
            <SecHeader title="Working Days / Roster" icon={Briefcase} />
            <Field label="Roster Type">
              <select className={selectCls} value={form.rosterType || 'Monday to Friday'} onChange={e => set('rosterType', e.target.value)}>
                {['Monday to Friday', 'Monday to Saturday', 'Custom'].map(r => <option key={r}>{r}</option>)}
              </select>
            </Field>
            <div className="grid grid-cols-2 gap-4">
              <Field label="Shift Start Time">
                <input type="time" className={inputCls} value={form.shiftStart || '09:00'} onChange={e => set('shiftStart', e.target.value)} />
              </Field>
              <Field label="Shift End Time">
                <input type="time" className={inputCls} value={form.shiftEnd || '18:00'} onChange={e => set('shiftEnd', e.target.value)} />
              </Field>
            </div>
            {form.rosterType === 'Custom' && (
              <div className="col-span-full">
                <label className="block text-xs font-semibold text-gray-600 dark:text-gray-400 mb-2 tracking-wide uppercase">Custom Working Days</label>
                <div className="flex flex-wrap gap-2">
                  {['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday'].map(day => {
                    const days = form.customDays || []
                    const checked = days.includes(day)
                    return (
                      <label key={day} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl border text-xs font-medium cursor-pointer transition-all ${checked ? 'bg-indigo-600 border-indigo-600 text-white' : 'border-gray-200 dark:border-gray-600 text-gray-600 dark:text-gray-300 hover:border-indigo-300'}`}>
            ...
            {/* Local Holidays */}
            <SecHeader title="Staff-Specific Local Holidays" icon={FileText} />
            <div className="col-span-full">
              <p className="text-xs text-gray-500 dark:text-gray-400 mb-3">Add holidays that apply only to this staff member (e.g. local/regional holidays).</p>
              <div className="space-y-2 mb-3">
                {(form.localHolidays || []).map((h, idx) => (
                  <div key={idx} className="flex gap-2 items-center">
                    <input type="date" className={inputCls + ' flex-1'} value={h.date} onChange={e => {
                      const arr = [...(form.localHolidays || [])]
                      arr[idx] = { ...arr[idx], date: e.target.value }
                      set('localHolidays', arr)
                    }} />
                    <input className={inputCls + ' flex-[2]'} placeholder="Holiday Name" value={h.name} onChange={e => {
                      const arr = [...(form.localHolidays || [])]
                      arr[idx] = { ...arr[idx], name: e.target.value }
                      set('localHolidays', arr)
                    }} />
                    <button type="button" onClick={() => {
                      const arr = (form.localHolidays || []).filter((_, i) => i !== idx)
                      set('localHolidays', arr)
              ...
              <button type="button" onClick={() => set('localHolidays', [...(form.localHolidays || []), { date: '', name: '' }])}
                className="flex items-center gap-2 px-3 py-2 text-xs font-semibold text-indigo-600 dark:text-indigo-400 border border-indigo-200 dark:border-indigo-700 rounded-xl hover:bg-indigo-50 dark:hover:bg-indigo-900/30 transition-all">
...
function StaffProfile({ staff, onClose, onEdit }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[200] flex items-start justify-center p-4 overflow-y-auto print-hide-wrapper">
      <div className="staff-print-area bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-4xl my-8">
        ...
        <div className="bg-gradient-to-r from-indigo-700 via-indigo-600 to-purple-700 rounded-t-2xl p-6 text-white relative">
          <button onClick={onClose} className="absolute top-4 right-4 p-2 hover:bg-white/20 rounded-xl transition-all print-hide">
            <X size={18} />
          </button>
        ...
        {/* Footer */}
        <div className="flex items-center justify-between p-6 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50 rounded-b-2xl print-hide">
          <button onClick={onClose}
