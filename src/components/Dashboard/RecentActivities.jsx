  ...
  return (
    <div className="card p-5">
      <div className="flex items-center justify-between mb-4">
        ...
        <button className="text-xs text-orange-600 dark:text-orange-400 font-medium hover:underline">View all</button>
      </div>
      <div className="space-y-3 max-h-72 overflow-y-auto pr-1">
        {recentActivities.map((activity) => (
          <div key={activity.id} className="flex items-start gap-3 group">
            <div className="w-8 h-8 rounded-xl bg-orange-50 dark:bg-orange-950/50 flex items-center justify-center text-sm shrink-0 group-hover:scale-110 transition-transform">
              {activity.icon}
            </div>
